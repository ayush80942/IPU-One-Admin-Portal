...
import { createNotice, CATEGORIES, BADGES, NoticeRequest } from "./lib/api";
...
const BATCH_YEAR_OPTIONS = Array.from({ length: 8 }, (_, i) => {
  const y = String(currentYear - i);
  return { value: y, label: y };
});

const ADMISSION_YEAR_OPTIONS = Array.from({ length: 8 }, (_, i) => {
  const y = String(currentYear - i);
  return { value: y, label: y };
});
...
export default function PublishNoticePage() {
  ...
  return (
    <div className="max-w-4xl">
      <h1 className="text-2xl font-bold text-primary">Publish Notice</h1>
      <p className="text-[14px] text-muted mt-1 mb-7">Create and publish a notice to targeted student groups.</p>
